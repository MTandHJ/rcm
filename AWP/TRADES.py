...
METHOD = "TRADES"
SAVE_FREQ = 5
FMT = "{description}={scale_factor}-{mode}-{strides}" \
        "={awp_gamma}-{awp_warmup}={leverage}={learning_policy}-{optimizer}-{lr}-{weight_decay}" \
...
parser.add_argument("--strides", type=str, default="1222",
                help="the stride for each block, default=1222 for resnet")
...
def load_cfg() -> Tuple[Config, str]:
    ...
    model = ADArch(
        ...
        scale_factor=opts.scale_factor, mode=opts.mode
    )

    # the proxy
    proxy = load_model(opts.model)(
        num_classes=get_num_classes(opts.dataset),
        strides=opts.strides
    )
    mean, std = load_normalizer(opts.dataset)
    proxy = ADArch(
        model=proxy, mean=mean, std=std,
        scale_factor=opts.scale_factor, mode=opts.mode
    )
